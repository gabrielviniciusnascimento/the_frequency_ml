#!/usr/bin/env python3
"""
Nome: grip_02_pipeline.py
Tarefa: Rodar no GRIP o MESMO pipeline da audiometria (scripts/26_method_comparison.py):
        espaço bilateral row-centered -> RobustScaler -> PCA95 -> K-means/GMM/HDBSCAN,
        mesmas seeds e grade. NÃO adaptar hiperparâmetros ao grip — divergência é achado.

Espaço de feature (análogo aos 14 limiares): os 6 trials brutos
  grip_R_t1,t2,t3 + grip_L_t1,t2,t3  (lado ANATÔMICO já reconstruído via MGATHAND no grip_01).

Filtros (espelham a audiometria onde há justificativa):
  - idade 20-69 (mesmo da audiometria).
  - completude: 6 trials válidos (análogo a >=10/14).
  - ANY25-equivalente: NÃO aplicado. O codebook MGX não define corte de grip
    anormal/inválido; cutoffs de fraqueza são de literatura externa de sarcopenia,
    não do codebook. Não inventar filtro.

Output: outputs/json/grip_02_pipeline.json
"""
...
# Parâmetros IDÊNTICOS ao 26_method_comparison.py
RANDOM_STATE = 42
...
GRIP_COLS = [f"grip_R_t{i}" for i in (1, 2, 3)] + [f"grip_L_t{i}" for i in (1, 2, 3)]
FEATURE = Path("data/processed/grip_feature_matrix.csv")
OUTPUT = Path("outputs/json/grip_02_pipeline.json")
LOG = Path("outputs/logs/grip_02_pipeline.log")
...
def load_pca_space():
    ...
    g = df[GRIP_COLS].apply(pd.to_numeric, errors="coerce")
    complete = g.notna().all(axis=1)
    g = g[complete]
    log.info(f"GRIP 20-69 com 6 trials válidos: {len(g)} indivíduos × 6")
    X = g.sub(g.mean(axis=1), axis=0).to_numpy(np.float64)  # row-center (remove nível)
    X = RobustScaler(quantile_range=(25, 75)).fit_transform(X)
    pca = PCA(n_components=PCA_VAR, random_state=RANDOM_STATE)
    Xp = pca.fit_transform(X)
    log.info(f"PCA: {Xp.shape[1]} comp (var={pca.explained_variance_ratio_.sum():.4f})")
...
def gap_statistic(X, k, b=GAP_B):
    ...
    refs = np.array([np.log(inertia(rng.uniform(mins, maxs, size=X.shape), RANDOM_STATE + i, 1))
                     for i in range(b)])
...
def gmm_block(X):
    per_k = {}
    for k in K_RANGE:
        gm = GaussianMixture(k, covariance_type="full", random_state=RANDOM_STATE, n_init=2, max_iter=200).fit(X)
        per_k[k] = {"bic": round(float(gm.bic(X)), 1)}
...
def gmm_cov_robustness(X):
    out = {}
    for cov in ["full", "tied", "diag", "spherical"]:
        bics = {k: float(GaussianMixture(k, covariance_type=cov, random_state=RANDOM_STATE,
                                         n_init=10, max_iter=300).fit(X).bic(X)) for k in K_RANGE}
        vals = [bics[k] for k in K_RANGE]
        bmin = K_RANGE[int(np.argmin(vals))]
        out[cov] = {"bic_min_k": int(bmin), "interior_minimum": bool(bmin not in (K_RANGE[0], K_RANGE[-1])),
                    "bic_range_pct": round(float((max(vals) - min(vals)) / np.mean(vals) * 100), 3)}
    interior = [c for c in out if out[c]["interior_minimum"]]
    out["_interior_in"] = interior
    return out
...
def main():
    ...
    gm["bic_robustness_covariance"] = gmm_cov_robustness(X)
    result = {
        "script": "grip_02_pipeline.py",
        "note": "MESMO pipeline da audiometria (26). Feature space = 6 grip trials (lado anatômico). "
                "Sem filtro ANY25-equivalente (não justificado no codebook MGX).",
        "n_samples": n, "pca_components": int(X.shape[1]),
        "kmeans": km, "gmm": gm, "hdbscan": hd,
        "comparison_to_audiometry": {
            "audiometry_best_silhouette": 0.28, "audiometry_hdbscan_largest_frac": 0.9223,
            "grip_best_silhouette": km["best_silhouette_value"],
            "grip_hdbscan_largest_frac": hd["largest_cluster_fraction"],
            "grip_gmm_bic_depth_pct": gm["bic_range_pct_of_mean"],
        },
        "status": "EXECUTED",
    }
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"\nGRIP pipeline: N={n}, PCA={X.shape[1]} comp")
    print(f"  K-means: best silhouette={km['best_silhouette_value']} @k={km['best_k_by_silhouette']}; gap-opt k={km['gap_optimal_k']}")
    print(f"  GMM: BIC min k={gm['bic_min_k']} interior={gm['interior_minimum']} depth={gm['bic_range_pct_of_mean']}% | interior cov: {gm['bic_robustness_covariance']['_interior_in']}")
