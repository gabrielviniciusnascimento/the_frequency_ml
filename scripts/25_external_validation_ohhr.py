...
def main():
    ...
    wide = pts.pivot_table(
        index="clientid", columns="frequency", values="level", aggfunc="first"
    ).reset_index()
    ...
    df["PTA"] = df[COMMON_FREQ_COLS].apply(pd.to_numeric, errors="coerce").mean(axis=1)
